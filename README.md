...
Raspberry Pi zur Auslesung des Landis+Gyr UH50 Wärmemengenzählers, Erfassung von Fritz!Box-, Wetter- und Steckdosendaten sowie System-Monitoring. Alle Werte werden über das Netzwerk in eine externe InfluxDB geschrieben und dort in Grafana visualisiert — beides läuft **nicht** auf diesem Pi.
...
| Komponente | Zweck |
|---|---|
| InfluxDB 2.8.0 *(extern)* | Zeitreihendatenbank für alle Messwerte — läuft nicht auf diesem Pi |
| Grafana *(extern)* | Dashboard und Visualisierung — läuft nicht auf diesem Pi |
...
| wlanthermo_influx.py | WLANThermo Nano V3 (BBQ) auslesen — Dauerdienst, kein Cronjob |
...
| `bbq` | WLANThermo Nano V3: Kanaele, Pitmaster, Geraetestatus | Forever |
...
Holt aktuelle Wetterdaten von Open-Meteo (kostenlos, kein API-Key) für Frankfurt (50.1109, 8.6821).
...
### `/home/admin/home-data-collection/wlanthermo/wlanthermo_influx.py`
Pollt die lokale HTTP-API des WLANThermo Nano V3 (`http://nanov3/data`) alle 10 Sekunden und
schreibt nach InfluxDB (Bucket: `bbq`). Laeuft als **systemd-Dienst**, nicht per Cron — das
Intervall liegt unter einer Minute. Ist der Nano aus, passiert nichts.
...
Der Tag `cook` traegt eine Session-ID; eine neue Session beginnt, wenn das Geraet laenger als
30 Minuten nicht erreichbar war. Kanaele ohne gesteckten Fuehler (`temp == 999`) werden nicht
geschrieben. Schreibfehler landen im Spool und werden nachgeschoben.

Details: `wlanthermo/README.md`
...
# InfluxDB (läuft extern, nicht auf diesem Pi)
INFLUX_URL   = https://dein-vps.example.com
INFLUX_TOKEN = ...
INFLUX_ORG   = home
...
| `/home/admin/home-data-collection/wlanthermo/wlanthermo_influx.py` | WLANThermo Nano V3 |
...
| `/home/admin/wlanthermo_data/` | State + Spool WLANThermo (Log via journalctl) |
...
> InfluxDB und Grafana laufen extern und werden hier nicht verwaltet. Auf diesem Pi laufen Telegraf und der WLANThermo-Collector.

```bash
# Status prüfen
sudo systemctl status telegraf
sudo systemctl status wlanthermo-collector

# Neu starten
sudo systemctl restart telegraf
sudo systemctl restart wlanthermo-collector

# Log des WLANThermo-Collectors
...
InfluxDB und Grafana sind extern gehostet (nicht auf diesem Pi) — URL siehe die jeweilige eigene Instanz.
...
| Wetter Frankfurt | `wetter-frankfurt` | Temperatur, Wind, Niederschlag, Luftdruck |
...
| BBQ – Aktueller Cook | `bbq-live-v1` | Live: Verlauf, Lüfter-Stellwert, Anstieg °C/h (Stall-Erkennung) |
| BBQ – Cooks | `bbq-cooks-v1` | Historie: Tabelle je Cook, Vergleich über „Stunden seit Start" |

---

## Backup wiederherstellen

> **Hinweis:** Dieser Abschnitt beschreibt den Stand, als InfluxDB noch lokal auf diesem Pi lief. Seit der Migration von InfluxDB und Grafana auf ein externes System laufen Backup/Restore dort — die folgenden Schritte sind nur noch historische Referenz.
